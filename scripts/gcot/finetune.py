#!/usr/bin/env python
"""
GCoT DownPrompt few-shot node classification: ConditionNet think + prototype matching; reuses MDGPT split paths.
"""
...
from pygfm.baseline_models import GCoTPrePromptModel, GCoTDownPromptModel
...
def _parse_args():
    p = argparse.ArgumentParser(
        description="GCoT DownPrompt few-shot node finetune",
    ...
    p.add_argument(
        ...
        type=str,
        default=None,
        help="PrePrompt checkpoint; if omitted, auto-detect under ckpts/gcot/ by dataset",
    )
...
def main():
    ...
    args.ckpt = resolve_preprompt_ckpt(ROOT, "gcot", args.dataset, args.ckpt)
    set_seed(args.seed)
    ...
    splits_path = args.splits_path or os.path.join(args.downstream_root, args.dataset, f"{args.k_shot}shot", "splits.pt")
    ...
    for sid in split_ids:
        ...
        best_loss = 1e9
        cnt_wait = 0
        print(f">> GCoT Finetune {args.dataset} | {args.k_shot}-shot | split {sid} | support={len(support_idx)}, test={len(test_idx)}")
