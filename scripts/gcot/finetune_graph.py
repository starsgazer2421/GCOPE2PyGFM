#!/usr/bin/env python
"""
GCoT DownPrompt few-shot graph classification; reuses MDGPT graph_batch split paths.
"""
...
from pygfm.baseline_models import GCoTPrePromptModel, GCoTDownPromptGraphModel
...
def _parse_args():
    p = argparse.ArgumentParser(description="GCoT DownPrompt few-shot graph finetune")
    ...
    p.add_argument("--data_root", type=str, default="datasets/gcot")
    ...
    p.add_argument("--condition_hid_dim", type=int, default=128)
    p.add_argument("--think_layer_num", type=int, default=1)
    p.add_argument("--condition_layer_num", type=int, default=1)
...
def main():
    ...
    splits_path = args.splits_path or os.path.join(args.downstream_root, args.dataset, f"{args.k_shot}shot_graph_batch", "splits.pt")
    ...
    testlist, testindex = build_test_subgraphs(
        edge_index.cpu(), test_indices, max_one_hop=args.max_one_hop, max_two_hop=args.max_two_hop, seed=args.seed
    )
    ...
    for sid in split_ids:
        ...
        down = GCoTDownPromptGraphModel(
            gcn=gcn,
            input_dim=unify_dim,
            hidden_dim=hidden_dim,
            num_classes=num_classes,
            think_layer_num=args.think_layer_num,
            condition_layer_num=args.condition_layer_num,
            condition_hid_dim=args.condition_hid_dim,
            device=device,
        )
        ...
        print(f">> GCoT Finetune graph {args.dataset} | {args.k_shot}-shot | split {sid} | support_graphs={num_support_graphs}, test_graphs={len(test_indices)}")
        ...
        for step in range(args.max_steps):
            ...
            logits = down(
                x, edge_index,
                support_idx=support_idx, support_batch=support_batch, support_labels=support_labels,
                query_idx=support_idx, query_batch=support_batch, train=True,
            )
        ...
        with torch.inference_mode():
            logits = down(
                x, edge_index,
                support_idx=support_idx, support_batch=support_batch, support_labels=support_labels,
                query_idx=testlist, query_batch=testindex, train=False,
            )
        acc = (logits.argmax(dim=1) == test_labels).float().mean().item()
        acc_list.append(acc)
        print(f"[{args.dataset}] graph {args.k_shot}-shot split {sid} test accuracy: {acc:.4f}")
        if use_swanlab:
            try:
    ...
    if len(acc_list) > 1:
        ...
        print(f"[{args.dataset}] graph {args.k_shot}-shot {len(acc_list)} splits mean acc: {mean_acc:.4f}, std: {std_acc:.4f}")
        if use_swanlab:
            try:
