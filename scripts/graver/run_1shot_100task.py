#!/usr/bin/env python
"""
Batch GRAVER 1-shot 100-task node and graph classification.

  python scripts/graver/run_1shot_100task.py
  python scripts/graver/run_1shot_100task.py --datasets Cora,Pubmed --no_swanlab
"""
...
def main():
    p = argparse.ArgumentParser(description="GRAVER batch 1-shot finetune")
    p.add_argument("--datasets", type=str, default=None)
    p.add_argument("--task_num", type=int, default=100)
    p.add_argument("--ckpt_dir", type=str, default="ckpts/graver")
    ...
    for ds in dss:
        ...
        if run_node:
            ...
            if not sp.exists():
                ...
                subprocess.run(
                    [
                        sys.executable,
                        str(ROOT / "scripts" / "graver" / "finetune.py"),
                        "--dataset", ds,
                        "--k_shot", "1",
                        "--ckpt", str(ckpt),
        ...
        if run_graph:
            ...
            if not gp.exists():
                print(f"[SKIP] {ds} graph: {gp} missing")
            else:
                print(f"\n>> {ds} graph 1-shot x{args.task_num}")
                subprocess.run(
                    [
                        sys.executable,
                        str(ROOT / "scripts" / "graver" / "finetune_graph.py"),
                        "--dataset", ds,
                        "--k_shot", "1",
                        "--ckpt", str(ckpt),
                        "--task_num", str(args.task_num),
