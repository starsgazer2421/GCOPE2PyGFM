#!/usr/bin/env python
"""Batch GraphKeeper 1-shot node + graph classification finetune."""
...
def main():
    ...
    p.add_argument("--ckpt_dir", type=str, default="ckpts/graphkeeper")
    ...
    for ds in dss:
        ...
        if run_node:
            ...
            if not sp.exists():
                ...
                print(f"\n>> {ds} GraphKeeper node 1-shot x{args.task_num}")
                subprocess.run(
                    [
                        sys.executable,
                        str(ROOT / "scripts" / "graphkeeper" / "finetune.py"),
                        "--dataset",
                        ds,
                        "--k_shot",
                        "1",
        ...
        if run_graph:
            ...
            if not gp.exists():
                ...
                print(f"\n>> {ds} GraphKeeper graph 1-shot x{args.task_num}")
                subprocess.run(
                    [
                        sys.executable,
                        str(ROOT / "scripts" / "graphkeeper" / "finetune_graph.py"),
                        "--dataset",
                        ds,
                        "--k_shot",
                        "1",
