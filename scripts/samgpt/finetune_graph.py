#!/usr/bin/env python
"""
SAMGPT DownPrompt few-shot graph classification: composed + open prompt; reuses MDGPT graph_batch split paths.
"""
...
from pygfm.baseline_models import SAMGPTPrePromptModel, SAMGPTDownPromptGraphModel
...
def _parse_args():
    p = argparse.ArgumentParser(description="SAMGPT DownPrompt few-shot graph finetune")
...
def main():
    ...
    if not os.path.isfile(splits_path):
        raise FileNotFoundError(
            ...
            "Run first: python scripts/mdgpt/generate_downstream.py graph_batch --dataset ... --k_shot ..."
        )
    ...
    for sid in split_ids:
        ...
        down = SAMGPTDownPromptGraphModel(
            gcn=gcn,
            input_dim=unify_dim,
            hidden_dim=hidden_dim,
        ...
        print(f">> SAMGPT Finetune graph {args.dataset} | {args.k_shot}-shot | split {sid} | support_graphs={num_support_graphs}, test_graphs={len(test_indices)}")
