...
from .graph_utils import k_hop_nb_graph
from .distributed_compat import process_on_master_and_sync_by_pickle
from .runtime import logger, pickle_save, time_logger
...
def calc_approximate_ppr_rank(
    g, alpha, n_rank, cache_template, topk, eps=1e-4, **kwargs
):
...
def topk_approximate_ppr_matrix(
    g, alpha, eps, topk, cache_template, normalization="row"
):
    ...
    def _topk_approximate_ppr_matrix(cache_file):
        ...
        col = col.cpu().numpy() if hasattr(col, "cpu") else np.asarray(col)
        ...
        nodes = g.nodes()
        idx = nodes.cpu().numpy() if hasattr(nodes, "cpu") else np.asarray(nodes)
        ...
        pickle_save(topk_matrix, cache_file)
...
def find_top_k_neighbors_within_khop_ego_subgraph_iter(
    ...
    for i in range(g.number_of_nodes()):
        ...
        if len(nb_list[i]) > 0:
            logger.warning(f"Node {i} has no neighbors")
...
def find_top_k_neighbors_within_khop_ego_subgraph(
    ...
    if k == 0:  # Save empty list if no neighbors
        pickle_save([[] for _ in g.nodes()], cache_file)

    # Step 1: Use dgl.khop_graph to find max_hops neighbors for all nodes
    logger.info(f"Start sorting top building PPR sorted neighbors within {max_hops} hop")
    ...
    logger.info(f"Created score matrix for ranking")
