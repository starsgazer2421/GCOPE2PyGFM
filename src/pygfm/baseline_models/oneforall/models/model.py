...
from pygfm.baseline_models.oneforall.gp.nn.layer.pyg import RGCNEdgeConv
from pygfm.baseline_models.oneforall.gp.nn.models.GNN import MultiLayerMessagePassing
from pygfm.baseline_models.oneforall.gp.nn.models.util_model import MLP
from pygfm.baseline_models.oneforall.gp.utils.utils import load_pretrained_state
from pygfm.public.utils.llm.dev_llm_stub import (
    RandomLlmStub,
    build_stub_tokenizer,
    is_random_llm_enabled,
)

# ``stub``: skip HuggingFace weights; use RandomLlmStub (smoke / offline; same 768 dim as ST)
LLM_DIM_DICT = {
    "ST": 768,
    "BERT": 768,
    "e5": 1024,
...
class LLMModel(torch.nn.Module):
    ...
    """
    def __init__(self, llm_name, quantization=True, peft=True, cache_dir=None, max_length=500):
        super().__init__()
        assert llm_name in LLM_DIM_DICT.keys()
        self.llm_name = llm_name
        self.quantization = quantization
        self._random_llm_stub = is_random_llm_enabled() or str(llm_name).lower() == "stub"

        self.indim = LLM_DIM_DICT[self.llm_name]
        if cache_dir is None:
            from pygfm.baseline_models.oneforall.paths import get_model_cache_dir

            cache_dir = str(get_model_cache_dir())
        self.cache_dir = cache_dir
        self.max_length = max_length
        if self._random_llm_stub:
            if peft:
                raise ValueError(
                    "GFM_RANDOM_LLM=1 is incompatible with peft=True; use peft=False for stub runs."
                )
            self.model = RandomLlmStub(self.indim)
            self.tokenizer = build_stub_tokenizer(max_length=max_length)
        else:
    ...
    def find_all_linear_names(self, model):
        ...
        """
        if getattr(self, "_random_llm_stub", False):
            return []
