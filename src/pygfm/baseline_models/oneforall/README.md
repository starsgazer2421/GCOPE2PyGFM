> **GFM-Toolbox port**: this directory is an in-package copy of the upstream code. Entry points and path conventions are in **[../../../docs/oneforall/README.md](../../../docs/oneforall/README.md)**; SwanLab / WandB are covered under **Experiment logging (SwanLab)** in that doc.
...
If `HF_ENDPOINT` is unset, `run_cdm.py` and `utils.py` default to [HF-Mirror](https://hf-mirror.com/) (`https://hf-mirror.com`). **Set this before importing `pytorch_lightning`** (handled at the very top of `run_cdm.py`); otherwise Lightning may load `huggingface_hub` first and pin the official endpoint. To use the official hub, run before launch:

```
export HF_ENDPOINT=https://huggingface.co
```

SwanLab is listed under `pip` in `environment.yml`. You can also install it with `pip install swanlab`.

### Experiment tracking (Wandb / SwanLab)

By default, training uses **Weights & Biases** (`logger_backend: wandb` in `configs/default_config.yaml`). To use **[SwanLab](https://swanlab.cn)** instead, set `logger_backend: swanlab` in that file or pass it on the command line:

```
python run_cdm.py logger_backend swanlab
```

Cloud uploads require SwanLab login (`swanlab login`). **Why it stays “offline”:** `configs/default_config.yaml` sets `offline_log: True`; for SwanLab this maps to **`mode=local`** (logs on disk, no cloud UI sync) unless you override it. **To use online SwanLab (cloud dashboard):** set `offline_log` to `False` **and** `swanlab_mode` to `cloud`, for example:

```
python run_cdm.py logger_backend swanlab offline_log False swanlab_mode cloud
```

For local-only logging, keep `offline_log True` or pass `swanlab_mode local`. The SwanLab project name defaults to **`ofa-test`** (`swanlab_project`). Optional: `swanlab_workspace`, `swanlab_logdir`, `swanlab_description`. To turn off external loggers entirely, use `logger_backend none`.
