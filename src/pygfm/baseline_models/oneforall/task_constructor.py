...
from pygfm.baseline_models.oneforall.paths import get_cache_root, low_resource_split_json
from pygfm.baseline_models.oneforall.data.KG.gen_data import KGOFADataset
from pygfm.baseline_models.oneforall.data.single_graph.gen_data import SingleGraphOFADataset
from pygfm.baseline_models.oneforall.fs_datamanager import SimpleFSManager
from pygfm.baseline_models.oneforall.gp.lightning.data_template import DataWithMeta
from pygfm.baseline_models.oneforall.ofa_datasets import (MultiDataset, GraphListHierDataset, SubgraphHierDataset,
...
from pygfm.baseline_models.oneforall.gp.lightning.metric import (binary_auc_func, flat_binary_func, classification_func, EvalKit, )
from pygfm.baseline_models.oneforall.utils import (binary_apr_func, binary_auc_multi_func, binary_single_auc_func, classification_single_func,
                   flat_auc, )


def resolve_ofa_dataset_class(dataset_name: str) -> type:
    """Lazy-import dataset classes per task; load chemmol only for molecular tasks (needs ``datasets/.../chemmol/id2element.csv``, etc.)."""
    if dataset_name in ("arxiv", "Cora", "Pubmed", "wikics"):
        return SingleGraphOFADataset
    if dataset_name in ("WN18RR", "FB15K237"):
        return KGOFADataset
    if dataset_name in ("chemblpre", "chempcba", "chemhiv"):
        from pygfm.baseline_models.oneforall.data.chemmol.gen_data import MolOFADataset

        return MolOFADataset
    raise KeyError(f"Unknown OFA dataset_name for UnifiedTaskConstructor: {dataset_name!r}")
...
def ArxivFSSplitter(dataset):
    ...
    with open(low_resource_split_json(), "r") as f:
...
class UnifiedTaskConstructor:
    def __init__(self, tasks: list[str], load_texts: bool, encoder: utils.SentenceEncoder,
                 task_config_lookup: dict, data_config_lookup: dict, root=None,
        ...
        Args:
            ...
            root: dataset loading directory (default: ``paths.get_cache_root()``)
            batch_size: int
            sample_size: int, -1 means full dataste
            eval_batch_size: int, None means equal to batch_size.
        """
        self.root = root if root is not None else str(get_cache_root())
        ...
        # low_resource_split.json: lazy read by few splitters; do not open in __init__.
    ...
    def get_ofa_data(self, dataset_config):
        ...
        if dataset_name not in self.dataset:
            self.dataset[dataset_name] = resolve_ofa_dataset_class(dataset_name)(
                dataset_name, load_texts=self.load_texts, root=self.root, encoder=self.encoder
            )
