...
from pygfm.baseline_models.oneforall.gp.utils.io import load_yaml
from pygfm.baseline_models.oneforall.paths import get_exp_root
...
def setup_exp(params):
    exp_root = str(get_exp_root())
    if not osp.exists(exp_root):
        os.makedirs(exp_root, exist_ok=True)

    curtime = datetime.now()
    exp_dir = osp.join(exp_root, str(curtime))
    os.mkdir(exp_dir)
...
def load_pretrained_state(model_dir, deepspeed=False):
    if deepspeed:
        ...
        state_dict = torch_load_compat(model_dir)["state_dict"]
    return state_dict
