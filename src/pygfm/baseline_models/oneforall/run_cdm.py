...
from pygfm.baseline_models.oneforall.gp.lightning.module_template import ExpConfig
from pygfm.baseline_models.oneforall.gp.lightning.training import lightning_fit
...
from pygfm.baseline_models.oneforall.lightning_model import GraphPredLightning
from pygfm.baseline_models.oneforall.models.model import BinGraphModel, BinGraphAttModel
from pygfm.baseline_models.oneforall.models.model import PyGRGCNEdge
from pygfm.baseline_models.oneforall.task_constructor import UnifiedTaskConstructor
from pygfm.baseline_models.oneforall.utils import (
    SentenceEncoder,
    MultiApr,
    MultiAuc,
)
...
def build_experiment_logger(params):
    """Create a Lightning logger: Wandb (default), SwanLab, or disabled."""
    backend = getattr(params, "logger_backend", "wandb")
    if isinstance(backend, str):
        backend = backend.strip().lower()
    if backend in ("none", "false", "null", ""):
        return False
    ...
    if backend == "swanlab":
        try:
            ...
            raise ImportError(
                "logger_backend is swanlab but the swanlab package is not installed. "
                "Install with: pip install swanlab"
            ) from e

        swanlab_mode = getattr(params, "swanlab_mode", None)
        if swanlab_mode == "":
            swanlab_mode = None
        if swanlab_mode is None and getattr(params, "offline_log", False):
            # Align with Wandb offline: log locally without cloud sync
            swanlab_mode = "local"

        swanlab_project = getattr(params, "swanlab_project", None)
        kwargs = {
            "project": swanlab_project if swanlab_project else params.log_project,
            "experiment_name": params.exp_name,
            "save_dir": params.exp_dir,
        }
        if swanlab_mode:
            kwargs["mode"] = swanlab_mode
        workspace = getattr(params, "swanlab_workspace", None)
        if workspace:
            kwargs["workspace"] = workspace
        logdir = getattr(params, "swanlab_logdir", None)
        if logdir:
            kwargs["logdir"] = logdir
        description = getattr(params, "swanlab_description", None)
        if description:
            kwargs["description"] = description

        if kwargs.get("mode") in ("local", "offline", "disabled"):
            kwargs["resume"] = "never"
...
def main(params):
    ...
    0. Check GPU setting.
    """
    from pygfm.baseline_models.oneforall.paths import ensure_runtime_dirs
    ...
    """
    experiment_logger = build_experiment_logger(params)
