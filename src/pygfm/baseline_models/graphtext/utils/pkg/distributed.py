...
def initialize_deepspeed(cfg):
    ...
    if cfg.use_deepspeed:
        try:
            ...
            logger.warning(
                "Skipping DeepSpeed init (missing dependency or incomplete env: %s); forcing use_deepspeed=false. "
                "For single-GPU smoke tests set use_deepspeed=false, or install mpi4py.",
                e,
            )
