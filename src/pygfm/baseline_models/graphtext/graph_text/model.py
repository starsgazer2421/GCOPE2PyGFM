...
class GraphText(nn.Module):
    ...
    def __init__(self, cfg, data, logger):
        ...
        # # Load checkpoint (offline smoke: small random Llama + local tokenizer, no HF download)
        if bool(cfg.llm.get("offline_smoke", False)):
            from llm.smoke_llm import build_offline_smoke_llm_and_tokenizer

            self.tokenizer, self.llm = build_offline_smoke_llm_and_tokenizer(cfg, max_tgt_len)
        ...
        # Data related: single-step .loc to avoid pandas Copy-on-Write chained-assignment errors
        for id, row in data.label_info.iterrows():
            data.label_info.at[id, "label_name"] = self.tokenizer.decode(
                self.tokenizer(row.label_name).input_ids[1:]
            )
