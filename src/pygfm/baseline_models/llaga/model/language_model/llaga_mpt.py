...
class LlagaMPTConfig(MPTConfig):
    model_type = "llaga_mpt"


class LlagaMPTModel(LlagaMetaModel, MPTModel):
    config_class = LlagaMPTConfig

    def __init__(self, config: MPTConfig):
        config.hidden_size = config.d_model
        super(LlagaMPTModel, self).__init__(config)
    
    def embed_tokens(self, x):
        return self.wte(x)


class LlagaMPTForCausalLM(MPTForCausalLM, LlagaMetaForCausalLM):
    config_class = LlagaMPTConfig
    ...
    def __init__(self, config):
        ...
        self.transformer = LlagaMPTModel(config)
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, LlagaMPTModel):
...
AutoConfig.register("llaga_mpt", LlagaMPTConfig)
AutoModelForCausalLM.register(LlagaMPTConfig, LlagaMPTForCausalLM)
