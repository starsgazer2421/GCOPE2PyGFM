...
from ..paths import dataset_rel, resolve_cora_data_path
from ..pyg_dataset import (
    load_nd_structure_or_zeros,
    normalize_loaded_graph,
    synthesize_nc_samples_from_pyg,
)
from ..io_utils import load_dataset_pt as _load_dataset_pt
...
class ModelArguments:
    ...
    offline_smoke: bool = field(
        default=False,
        metadata={"help": "Use bundled fake OPT+tokenizer under fixtures/smoke_opt_llm (no HF Hub)."},
    )
...
class DataArguments:
    ...
    graph_pt_path: Optional[str] = field(
        default=None,
        metadata={"help": "Cora: override graph .pt path (otherwise resolve_cora_data_path)."},
    )
    pyg_center_text: str = field(
        default="",
        metadata={"help": "NC prompt placeholder when using PyG-only Cora.pt (no paper text)."},
    )
    node_feature_dim: int = field(
        default=1433,
        metadata={"help": "Input dim for pretrained_embedding_type=node_x (match data.x)."},
    )
...
class LazySupervisedGraphDataset(Dataset):
    ...
    def __init__(self,
        ...
        for d, dataset in enumerate(self.use_dataset):
            ...
            if "." in dataset:
                ...
                dataset=ds[0]
            if "arxiv" in dataset:
                data_path = dataset_rel("ogbn-arxiv", "processed_data.pt")
            elif "products"  in dataset:
                data_path = dataset_rel("ogbn-products", "processed_data.pt")
            elif "pubmed"  in dataset:
                data_path = dataset_rel("pubmed", "processed_data.pt")
            elif "cora" in dataset:
                data_path = resolve_cora_data_path(data_args.graph_pt_path or None)
            else:
                print(f"{dataset} not exists!!!!")
                raise ValueError
            raw = _load_dataset_pt(data_path)
            if dataset == "cora":
                data, is_pyg = normalize_loaded_graph(raw, dataset)
            else:
                data = raw
                is_pyg = False
            if is_pyg and data_args.template != "ND":
                raise ValueError(
                    "PyG-style Cora .pt (no label_texts) supports template=ND only; "
                    "use full dataset/cora layout for HO."
                )
            self.datas[dataset]=data
            data_dir=os.path.dirname(data_path)
            if data_args.template == "ND":
                if data_args.pretrained_embedding_type == "node_x":
                    if not hasattr(data, "x"):
                        raise ValueError("pretrained_embedding_type=node_x requires data.x (PyG compat).")
                    pretrained_emb = data.x.float()
                elif is_pyg:
                    raise ValueError(
                        "PyG Cora.pt: set pretrained_embedding_type=node_x to use data['x'] as node embeddings."
                    )
                else:
                    pretrained_emb = self.load_pretrain_embedding_graph(data_dir, data_args.pretrained_embedding_type)
                lap_p = dataset_rel(f"laplacian_{data_args.use_hop}_{data_args.sample_neighbor_size}.pt")
                self.structure_emb = load_nd_structure_or_zeros(
                    lap_p, data_args.use_hop, data_args.sample_neighbor_size
                )
                if not os.path.isfile(lap_p):
                    rank0_print(f"Missing laplacian {lap_p}; using zero structure embeddings.")
            elif data_args.template == "HO":
                if is_pyg:
                    raise ValueError("PyG-style Cora .pt supports template=ND only.")
            ...
            for task in self.use_task:
                if is_pyg and task != "nc":
                    raise ValueError(
                        f"PyG-style Cora.pt only supports node classification (nc); remove task {task!r} from use_task."
                    )
                task_list_data_dict = []
                if task == "nc":
                    if data_args.template == "HO":
                        jsonl_path = os.path.join(data_dir, f"sampled_2_10_train.jsonl")
                    else:
                    ...
                    if os.path.exists(jsonl_path):
                        ...
                        for l in synthesize_nc_samples_from_pyg(
                            data,
                            dataset,
                            data_args.use_hop,
                            data_args.sample_neighbor_size,
                            center_text=data_args.pyg_center_text,
                        ):
                            l["dataset"] = dataset
                            task_list_data_dict.append(l)
                    else:
                        raise ValueError(f"Missing training jsonl: {jsonl_path}")
                elif task == "lp":
                    if data_args.template == "HO":
    ...
    def load_pretrain_embedding_graph(self, data_dir, pretrained_embedding_type):
        if pretrained_embedding_type == "simteg":
            simteg_sbert = _load_dataset_pt(os.path.join(data_dir, "simteg_sbert_x.pt"))
            simteg_roberta = _load_dataset_pt(os.path.join(data_dir, "simteg_roberta_x.pt"))
            simteg_e5 = _load_dataset_pt(os.path.join(data_dir, "simteg_e5_x.pt"))
            pretrained_emb = torch.concat([simteg_sbert, simteg_roberta, simteg_e5], dim=-1)
        else:
            pretrained_emb = _load_dataset_pt(os.path.join(data_dir, f"{pretrained_embedding_type}_x.pt"))
        return pretrained_emb

    def load_pretrain_embedding_hop(self, data_dir, pretrained_embedding_type, hop):
        if pretrained_embedding_type == "simteg":
            simteg_sbert=[_load_dataset_pt(os.path.join(data_dir, f"simteg_sbert_x.pt"))] + [_load_dataset_pt(os.path.join(data_dir, f"simteg_sbert_{i}hop_x.pt")) for i in range(1, hop + 1)]
            simteg_roberta = [_load_dataset_pt(os.path.join(data_dir, f"simteg_roberta_x.pt"))] + [_load_dataset_pt(os.path.join(data_dir, f"simteg_roberta_{i}hop_x.pt")) for i in range(1, hop + 1)]
            simteg_e5 = [_load_dataset_pt(os.path.join(data_dir, f"simteg_e5_x.pt"))] + [_load_dataset_pt(os.path.join(data_dir, f"simteg_e5_{i}hop_x.pt")) for i in range(1, hop + 1)]
            pretrained_embs = [torch.cat([simteg_sbert[i], simteg_roberta[i], simteg_e5[i]], dim=-1) for i in range(hop + 1)]
        else:
            pretrained_embs = [_load_dataset_pt(os.path.join(data_dir, f"{pretrained_embedding_type}_x.pt"))]+  [_load_dataset_pt(os.path.join(data_dir, f"{pretrained_embedding_type}_{i}hop_x.pt")) for i in range(1, hop+1)]
...
def _report_to_backends(report_to) -> list:
    if report_to is None or (isinstance(report_to, str) and str(report_to).lower() == "none"):
        return []
    if isinstance(report_to, str):
        return [report_to.lower()]
    return [str(x).lower() for x in report_to]


def _set_training_args_report_to(training_args, backends: list) -> None:
    if len(backends) == 0:
        training_args.report_to = "none"
    elif len(backends) == 1:
        training_args.report_to = backends[0]
    else:
        training_args.report_to = backends


def prepare_swanlab_callback(training_args):
    """
    HuggingFace Trainer does not recognize 'swanlab' in --report_to.
    Use either:
      - export LLAGA_REPORT_TO=swanlab   (disables wandb from CLI when both were set)
      - --report_to swanlab
    Optional env: LLAGA_SWANLAB_PROJECT, LLAGA_SWANLAB_MODE (local|cloud|...),
    LLAGA_SWANLAB_EXPERIMENT_NAME, LLAGA_SWANLAB_WORKSPACE, LLAGA_SWANLAB_LOGDIR.
    """
    env = os.environ.get("LLAGA_REPORT_TO", "").strip().lower()
    backends = _report_to_backends(training_args.report_to)
    had_swanlab_cli = "swanlab" in backends
    wants_swanlab = env == "swanlab" or had_swanlab_cli
    backends = [b for b in backends if b != "swanlab"]
    if env == "swanlab":
        backends = [b for b in backends if b not in ("wandb", "tensorboard", "mlflow", "codecarbon")]
    _set_training_args_report_to(training_args, backends)
    if not wants_swanlab:
        return None
    try:
        from swanlab.integration.transformers import SwanLabCallback
    except ImportError as e:
        raise ImportError(
            "SwanLab requested (LLAGA_REPORT_TO=swanlab or --report_to swanlab) but swanlab is not installed. "
            "pip install swanlab"
        ) from e
    kwargs = {}
    if os.environ.get("LLAGA_SWANLAB_PROJECT"):
        kwargs["project"] = os.environ["LLAGA_SWANLAB_PROJECT"]
    elif os.environ.get("SWANLAB_PROJECT"):
        kwargs["project"] = os.environ["SWANLAB_PROJECT"]
    else:
        kwargs["project"] = "llaga"
    if os.environ.get("LLAGA_SWANLAB_WORKSPACE"):
        kwargs["workspace"] = os.environ["LLAGA_SWANLAB_WORKSPACE"]
    if os.environ.get("LLAGA_SWANLAB_EXPERIMENT_NAME"):
        kwargs["experiment_name"] = os.environ["LLAGA_SWANLAB_EXPERIMENT_NAME"]
    if os.environ.get("LLAGA_SWANLAB_DESCRIPTION"):
        kwargs["description"] = os.environ["LLAGA_SWANLAB_DESCRIPTION"]
    if os.environ.get("LLAGA_SWANLAB_LOGDIR"):
        kwargs["logdir"] = os.environ["LLAGA_SWANLAB_LOGDIR"]
    if os.environ.get("LLAGA_SWANLAB_MODE"):
        kwargs["mode"] = os.environ["LLAGA_SWANLAB_MODE"]
...
def _train():
    ...
    if data_args.pretrained_embedding_type == "node_x" and data_args.template != "ND":
        raise ValueError("pretrained_embedding_type=node_x only supports template=ND.")
    ...
    elif data_args.pretrained_embedding_type == "node_x":
        model_args.mm_hidden_size = int(data_args.node_feature_dim)
    else:
        raise ValueError(
            f"Unknown pretrained_embedding_type: {data_args.pretrained_embedding_type!r}"
        )
    if data_args.template == "ND":
        if data_args.sample_neighbor_size <= 1:
            raise ValueError("ND template requires sample_neighbor_size > 1.")
        data_args.structure_embedding_dim = int(
    ...
    if model_args.offline_smoke or os.environ.get("LLAGA_OFFLINE_SMOKE") == "1":
        os.environ.setdefault("TRANSFORMERS_OFFLINE", "1")
        os.environ.setdefault("HF_HUB_OFFLINE", "1")
        from ..fixtures.smoke_bundle import ensure_smoke_llm_fixture

        fixture_root = ensure_smoke_llm_fixture()
        model_args.model_name_or_path = str(fixture_root)
        rank0_print(f"offline_smoke: using local fake LLM at {fixture_root}")
    ...
    trainer = LLaGATrainer(model=model,
                    tokenizer=tokenizer,
                    args=training_args,
                    callbacks=(prepare_swanlab_callback(training_args) or []),
                    **data_module)
