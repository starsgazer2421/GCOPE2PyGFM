...
def load_pretrain_embedding_graph(data_dir, pretrained_embedding_type):
    if pretrained_embedding_type == "simteg":
        simteg_sbert = load_dataset_pt(os.path.join(data_dir, "simteg_sbert_x.pt"))
        simteg_roberta = load_dataset_pt(os.path.join(data_dir, "simteg_roberta_x.pt"))
        simteg_e5 = load_dataset_pt(os.path.join(data_dir, "simteg_e5_x.pt"))
        pretrained_emb = torch.concat([simteg_sbert, simteg_roberta, simteg_e5], dim=-1)
    else:
        pretrained_emb = load_dataset_pt(os.path.join(data_dir, f"{pretrained_embedding_type}_x.pt"))
    return pretrained_emb

def load_pretrain_embedding_hop(data_dir, pretrained_embedding_type, hop, mask):
    if pretrained_embedding_type == "simteg":
        simteg_sbert=[load_dataset_pt(os.path.join(data_dir, f"simteg_sbert_x.pt"))[mask]] + [load_dataset_pt(os.path.join(data_dir, f"simteg_sbert_{i}hop_x.pt"))[mask] for i in range(1, hop + 1)]
        simteg_roberta = [load_dataset_pt(os.path.join(data_dir, f"simteg_roberta_x.pt"))[mask]] + [load_dataset_pt(os.path.join(data_dir, f"simteg_roberta_{i}hop_x.pt"))[mask] for i in range(1, hop + 1)]
        simteg_e5 = [load_dataset_pt(os.path.join(data_dir, f"simteg_e5_x.pt"))[mask]] + [load_dataset_pt(os.path.join(data_dir, f"simteg_e5_{i}hop_x.pt"))[mask] for i in range(1, hop + 1)]
        pretrained_embs = [torch.cat([simteg_sbert[i], simteg_roberta[i], simteg_e5[i]], dim=-1) for i in range(hop + 1)]
    else:
        pretrained_embs = [load_dataset_pt(os.path.join(data_dir, f"{pretrained_embedding_type}_x.pt"))[mask]]+  [load_dataset_pt(os.path.join(data_dir, f"{pretrained_embedding_type}_{i}hop_x.pt"))[mask] for i in range(1, hop+1)]

    return pretrained_embs

def load_pretrain_embedding_hop_lp(data_dir, pretrained_embedding_type, hop):
    mask = load_dataset_pt(os.path.join(data_dir, f"no_test_link_mask.pt"))
    if pretrained_embedding_type == "simteg":
        simteg_sbert=[load_dataset_pt(os.path.join(data_dir, f"simteg_sbert_x.pt"))[mask]] + [load_dataset_pt(os.path.join(data_dir, f"simteg_sbert_{i}hop_x_notestlink.pt")) for i in range(1, hop + 1)]
        simteg_roberta = [load_dataset_pt(os.path.join(data_dir, f"simteg_roberta_x.pt"))[mask]] + [load_dataset_pt(os.path.join(data_dir, f"simteg_roberta_{i}hop_x_notestlink.pt")) for i in range(1, hop + 1)]
        simteg_e5 = [load_dataset_pt(os.path.join(data_dir, f"simteg_e5_x.pt"))[mask]] + [load_dataset_pt(os.path.join(data_dir, f"simteg_e5_{i}hop_x_notestlink.pt")) for i in range(1, hop + 1)]
        pretrained_embs = [torch.cat([simteg_sbert[i], simteg_roberta[i], simteg_e5[i]], dim=-1) for i in range(hop + 1)]
    else:
        pretrained_embs = [load_dataset_pt(os.path.join(data_dir, f"{pretrained_embedding_type}_x.pt"))[mask]]+  [load_dataset_pt(os.path.join(data_dir, f"{pretrained_embedding_type}_{i}hop_x_notestlink.pt")) for i in range(1, hop+1)]
...
def eval_model(args):
    ...
    if args.dataset == "arxiv":
        data_dir = dataset_rel("ogbn-arxiv")
    elif args.dataset == "products":
        data_dir = dataset_rel("ogbn-products")
    elif args.dataset == "pubmed":
        data_dir = dataset_rel("pubmed")
    elif args.dataset == "cora":
        data_dir = dataset_rel("cora")
    else:
    ...
    data = load_dataset_pt(data_path)
    ...
    if args.template == "ND":
        ...
        structure_emb = load_dataset_pt(
            dataset_rel(f"laplacian_{args.use_hop}_{args.sample_neighbor_size}.pt"))
