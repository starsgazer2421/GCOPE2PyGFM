...
from .GraphLlama import GraphLlamaForCausalLM
import transformers
...
class GraphGPT_pl(LightningModule): 
    ...
    ## load 4 8 bit 
        ...
        # Align with train_graph.py: allow passing either a name (resolved under ckpts/graphgpt/checkpoints/)
        # or an explicit directory path.
        if not hasattr(self.model.config, "pretrain_graph_model_path"):
            self.model.config.pretrain_graph_model_path = ""
        if model_args.graph_tower is not None:
            import os
            import pathlib

            _tower = model_args.graph_tower
            _ckpt_root = pathlib.Path("ckpts") / "graphgpt" / "checkpoints"
            _enc = _ckpt_root / _tower
            if _enc.is_dir():
                self.model.config.pretrain_graph_model_path = str(_enc.resolve()) + os.sep
            elif pathlib.Path(_tower).is_dir():
                self.model.config.pretrain_graph_model_path = str(pathlib.Path(_tower).resolve()) + os.sep
            else:
                self.model.config.pretrain_graph_model_path = (
                    self.model.config.pretrain_graph_model_path or ""
                ) + _tower
