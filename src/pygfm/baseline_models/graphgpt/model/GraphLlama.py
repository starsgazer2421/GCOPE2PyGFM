...
from .graph_layers import MPNN, GNN, CLIP, graph_transformer
...
class GraphLlamaModel(LlamaModel):
    ...
    def initialize_graph_modules(self, graph_tower, graph_select_layer,
        ...
        if not hasattr(self.config, "graph_hidden_size"):
            _gt = self.get_graph_tower()
            _args = getattr(_gt, "args", None)
            if _args is not None and hasattr(_args, "gnn_output"):
                self.config.graph_hidden_size = int(_args.gnn_output)
            else:
        ...
        if pretrain_graph_mlp_adapter is not None:
            try:
                blob = torch.load(
                    pretrain_graph_mlp_adapter, map_location="cpu", weights_only=False
                )
            except TypeError:
                blob = torch.load(pretrain_graph_mlp_adapter, map_location="cpu")
            gp_sd = {
                k.split(".")[-1]: v
                for k, v in blob.items()
                if "graph_projector" in k
            }
            self.graph_projector.load_state_dict(gp_sd)
